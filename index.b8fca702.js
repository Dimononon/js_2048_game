...
function render() {
    ...
    activeTiles.forEach((tile)=>{
        ...
        if (tile.isNew) tileEl.classList.add("is-new");
        else tileEl.classList.remove("is-new");
        if (tile.isMerged) tileEl.classList.add("is-merged");
        else tileEl.classList.remove("is-merged");
...
document.addEventListener("keydown", (e)=>{
    ...
    if (e.key === "ArrowUp" || e.key.toLowerCase() === "w") game.moveUp();
    if (e.key === "ArrowDown" || e.key.toLowerCase() === "s") game.moveDown();
    if (e.key === "ArrowLeft" || e.key.toLowerCase() === "a") game.moveLeft();
    if (e.key === "ArrowRight" || e.key.toLowerCase() === "d") game.moveRight();
...
class Game {
    constructor(initialState){
        ...
        for(let r = 0; r < 4; r++){
            for(let c = 0; c < 4; c++)if (this.initialState[r][c] !== 0) {
                const tile = {
                    id: this.nextId++,
    ...
    addRandomCell() {
        const emptyCells = [];
        for(let r = 0; r < 4; r++){
            for(let c = 0; c < 4; c++)if (!this.state[r][c]) emptyCells.push({
                r,
                c
            });
        }
    ...
    checkEndGame() {
        ...
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
            if (arr[r][c] >= 2048) has2048 = true;
            if (arr[r][c] === 0) hasEmpty = true;
            if (c < 3 && arr[r][c] === arr[r][c + 1]) hasMoves = true;
            if (r < 3 && arr[r][c] === arr[r + 1][c]) hasMoves = true;
        }
        if (has2048) this.status = "win";
        else if (!hasEmpty && !hasMoves) this.status = "lose";
    }
    move(direction) {
        ...
        for(let i = 0; i < 4; i++){
            ...
            for(let j = 0; j < 4; j++){
                ...
                if (this.state[r][c] !== null) lineTiles.push(this.state[r][c]);
            }
            const newLine = [];
            let k = 0;
            while(k < lineTiles.length)if (k + 1 < lineTiles.length && lineTiles[k].value === lineTiles[k + 1].value) {
                lineTiles[k].value *= 2;
            ...
            for(let j = 0; j < 4; j++){
                ...
                if (this.state[r][c] !== item) moved = true;
                this.state[r][c] = item;
                if (item) {
                    item.r = r;
                    item.c = c;
